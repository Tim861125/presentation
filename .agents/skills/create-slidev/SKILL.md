...
- **References page** — 技術 deck 在 End 前加一頁 `layout: tech-content`（`eyebrow: References`、`title: 參考文獻`），內容用 `<div class="grid grid-cols-2 gap-3">` 包 `<ReferenceCard index title href />`；來源取自 spec.md 的「來源文件 / 參考連結」。不要手刻裸 `<a>` 連結清單或自創 slate/cyan 樣式。
