...
const cols = [
  {
    ...
    dot: 'bg-emerald-400',
    desc: '字面精確匹配',
    good: ['精確詞、型號、專有名詞命中強', '可解釋、可調權重', '不需模型、成本低'],
  ...
  },
  {
    ...
    bad: ['漏掉需要精確詞的匹配', '分數尺度與 BM25 完全不同'],
  },
]
</script>
    ...
    <div class="mt-3 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.06] px-5 py-3">
      <div class="flex items-center gap-3">
        ...
        <p class="text-sm text-zinc-200">
          同一查詢<span class="text-white font-semibold">同時跑多路子查詢</span>，把不同尺度的分數
          <span class="text-emerald-400 font-semibold">正規化 → 加權合併</span>成單一排名 —— 互補兩者盲點，relevance 更好。
        </p>
      </div>
    </div>
  </SlideShell>
