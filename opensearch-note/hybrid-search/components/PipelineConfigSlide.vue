...
const notes = [
  ...
  ['weights [0.4, 0.3, 0.3]', 'tac 給 0.4（重精確詞）、title / claims 各 0.3'],
]
</script>
...
<template>
  <SlideShell>
    <SlideHeader
      ...
      subtitle="專利場景：關鍵字命中 tac 權重最高，標題與請求項語意各分擔其餘權重。"
    />

    <div class="grid grid-cols-2 gap-5 flex-1">
      <JsonCard method="PUT" path="/_search/pipeline/patent-hybrid" :code="code" />

      <div class="flex flex-col gap-3">
        <div v-for="n in notes" :key="n[0]"
        ...
        <div class="mt-auto rounded-xl border border-emerald-500/20 bg-emerald-500/[0.06] px-4 py-3">
          <p class="text-sm text-zinc-200">
            <span class="text-emerald-400 font-semibold">關鍵：</span>
            weights 的順序，會與下一頁 hybrid query 中
            <span class="font-mono text-white">queries</span> 陣列的順序<span class="text-white font-semibold">一一對應</span>。
          </p>
        </div>
      </div>
    </div>
